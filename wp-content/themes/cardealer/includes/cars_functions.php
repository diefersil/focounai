...
if ( ! function_exists( 'cardealer_add_vehicale_to_cart' ) ) {
	...
	function cardealer_add_vehicale_to_cart( $vehicle_id ) {
		...
		if ( $is_vehicle_sell ) {
			...
			?>
			<div class="car-vehicle-buy-online add-vehicale-to-cart-btn">
				<button class="car-buy-online-btn button" harf="javascript:void();" data-id="<?php echo esc_attr( $vehicle_id ); ?>">
...
function cardealer_get_vehicle_stock( $vehicle_id = '' ) {
	...
	if ( empty( $vehicle_id ) ) {
		$post = get_post();
		if ( $post && is_a( $post, 'WP_Post' ) && 'cars' === $post->post_type ) {
			$vehicle_id = $post->ID;
		}
	}
	...
	return apply_filters( 'cardealer_get_vehicle_stock', $stock, $vehicle_id );
}
...
function cardealer_is_vehicle_sell_enabled( $vehicle_id = '' ) {
	...
	return apply_filters( 'cardealer_get_vehicle_stock', $enabled, $vehicle_id );
}
...
function sell_vehicle_lable_text() {
	...
	$lable_text              = $car_dealer_options['sell_vehicle_lable_text']; ;
	$sell_vehicle_lable_text = ( isset( $lable_text ) && ! empty( $lable_text ) ) ? $lable_text : esc_html__( 'Buy Online', 'cardealer' );
	...
	function cardealer_get_vehicle_review_stamps( $id = null, $echo = false ) {
		if ( null !== $id ) {
			...
			for ( $i = 1; $i <= $review_stamp_limit; $i++ ) {
				$review_stamp_logo = get_post_meta( $id, 'review_stamp_logo_' . $i, true );
				$review_stamp_link = '';
				$image_url         = '';
				if ( ! empty( $review_stamp_logo ) ) {
					$image_url         = wp_get_attachment_url( $review_stamp_logo );
				...
				if ( ! empty( $image_url ) ) {
					...
					if ( $echo ) {
						...
						if ( '' !== $review_stamp_link ) {
							$link .= '<a href="' . esc_url( $review_stamp_link ) . '" target="_blank"><img src="' . esc_url( $image_url ) . '" alt="carfax"/></a>';
	...
	function cardealer_car_price_html( $class = '', $id = null, $tax_label = true, $echo = true ) {
		...
		if ( ( $regular_price > 0 ) || ( $sale_price > 0 ) ) {
			...
			if ( $is_sell && ( ! $is_single || in_array( 'related-slider', $class_array) ) && ! in_array( 'hide-sell', $class_array ) ) {
				...
				$lable_text  = sell_vehicle_lable_text();
				$price_html .= '<div class="car-buy-online-btn" data-id="' . esc_attr( $car_id ) . '">';
...
if ( ! function_exists( 'cardealer_get_carplaceholder' ) ) {
	...
	function cardealer_get_carplaceholder( $size = '', $return_type = 'image' ) {
		...
		if ( isset( $_POST['action'] ) && 'cardealer_load_more_vehicles' === $_POST['action'] && 'image' === $return_type ) { // phpcs:ignore WordPress.Security.NonceVerification
...
if ( ! function_exists( 'cardealer_cars_content_class' ) ) {
	...
	function cardealer_cars_content_class( $extra_classes = '' ) {
		...
		if ( isset( $getlayout ) && ! empty( $getlayout ) ) {
			...
			} elseif ( ( 'view-list-left' === $getlayout ) || ( 'view-list-right' === $getlayout ) ) {
...
if ( ! function_exists( 'cardealer_grid_view_class' ) ) {
	...
	function cardealer_grid_view_class() {
		...
		$getlayout = cardealer_get_cars_list_layout_style();
		if ( wp_is_mobile() || ( isset( $getlayout ) && ! empty( $getlayout ) && 5 !== $columns ) ) {
			...
			<div class="grid-view change-view-button">
				<div class="view-icon">
					...
					foreach ( $list_view_layout as $key => $value ) {
						...
						$class            = ( strpos( $value, 'grid') !== false ) ? 'view-grid' : 'view-list';
						?>
...
if ( ! function_exists( 'cardealer_cars_catalog_ordering' ) ) :
	...
	function cardealer_cars_catalog_ordering( $args = array() ) {
		...
		if ( ( 'view-list-full' === $getlayout || 'view-grid-full' === $getlayout || 'lazyload' === $layout ) && $enable_ordering ) {
			?>
	...
	function cardealer_new_get_all_filters( $get_arg ) {
		...
		foreach ( $_GET as $gkey => $gval ) { // phpcs:ignore WordPress.Security.NonceVerification
			...
			if ( 'vehicle_location' === $gkey ) {
				$attributs .= '<li id="stripe-item-' . esc_attr( $gkey ) . '" data-type="' . esc_attr( $gkey ) . '" ><a href="javascript:void(0)"><i class="far fa-times-circle"></i> ' . esc_html__( 'Location', 'cardealer' ) . ' :  <span data-key="' . esc_attr( sanitize_text_field( wp_unslash( $_GET[ $gkey ] ) ) ) . '">' . esc_html( sanitize_text_field( wp_unslash( $_GET[ $gkey ] ) ) ) . '</span></a></li>'; // phpcs:ignore
			}
		}
			$attributs .= '</ul>';
		$attributs     .= '</div>';
...
if ( ! function_exists( 'cd_sell_car_online' ) ) {
	...
	function cd_sell_car_online() {
		...
		if ( ! isset( $_POST['ajax_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['ajax_nonce'] ) ), 'cd_sell_car_online' ) ) {
			$response = array(
				'status' => 'error',
