...
function cardealer_tweak_redux_field_sortable( $render, $field ) {
	if ( 'vehicle_detail_mobile_sections' === $field['id'] && class_exists( 'Redux' ) ) {
		$value = Redux::get_option( 'car_dealer_options', 'vehicle_detail_mobile_sections' );
		...
		foreach ( $value as $k => $nicename ) {
			...
			if ( 'checkbox' === $field['mode'] ) {
				...
				if ( 'checkbox' === $field['mode'] ) {
					echo '<label for="' . esc_attr( $field['id'] . '[' . $k ) . ']"><strong>' . wp_kses( wp_specialchars_decode( $options[ $k ] ), array( 'span' => array() ) ) . '</strong></label>';
				}
			}
