...
if ( ! function_exists( 'cardealer_load_style_script' ) ) {
	...
	function cardealer_load_style_script() {
		...
		// Google Map
		wp_register_script( 'cardealer-google-maps-apis', 'https://maps.googleapis.com/maps/api/js?key=' . $google_maps_api, array(), CARDEALER_VERSION, true );
		...
		wp_localize_script(
			...
			array(
				...
				'cd_sell_car_online'                       => wp_create_nonce( 'cd_sell_car_online' ),
		...
		if ( ( $enable_custom_banner && ! empty( $banner_type ) && 'video' === $banner_type ) || ( ! empty( $car_dealer_options['banner_type'] ) && 'video' === $car_dealer_options['banner_type'] ) ) {
			if ( cardealer_check_plugin_active( 'js_composer/js_composer.php' ) ) {
				...
				wp_enqueue_script( 'youtube_iframe_api_js', '//www.youtube.com/iframe_api', array(), CARDEALER_VERSION, false );
			}
		}
		...
		*/
		...
		if ( class_exists( 'WooCommerce' ) && is_product() ) {
			...
			if ( ! wp_script_is( 'select2', 'enqueued' ) ) {
				wp_enqueue_script( 'select2', WC()->plugin_url() . '/assets/js/select2/select2.full' . $suffix . '.js', array( 'jquery' ), '4.0.0', false );
			}
...
if ( ! function_exists( 'cardealer_admin_enqueue_scripts' ) ) {
	...
	function cardealer_admin_enqueue_scripts( $hook ) {
		...
		wp_register_script( 'cardealer_admin_js', CARDEALER_URL . '/js/admin.js', $cardealer_admin_js_deps, CARDEALER_VERSION, false );
		...
		wp_register_style( 'cardealer-css-jqueryui', CARDEALER_URL . '/css/jquery-ui/jquery-ui.min.css', array(), '1.12.1' );
