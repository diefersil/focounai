	...
	class Extended_CPT {
		...
		public function post_type_link( $post_link, WP_Post $post, $leavename, $sample ) {
			...
			foreach ( get_object_taxonomies( $post ) as $tax ) {
				if ( false === (bool) strpos( $post_link, "%{$tax}%" ) ) {
					continue;
				}
		...
		public function register_post_type() {
			...
			if ( ! isset( $this->args['query_var'] ) || ( true === (bool) $this->args['query_var'] ) ) {
			...
			$taxonomies = get_taxonomies( array( 'query_var' => $query_var ) );

			if ( $query_var && count( $taxonomies, 'objects' ) ) {
						...
						trigger_error(
							esc_html(
								sprintf(
									...
									__( 'Post type query var "%s" clashes with a taxonomy query var of the same name', 'cardealer-helper' ),
									$query_var
								)
							),
							E_USER_ERROR
		...
		public function add_taxonomy( $taxonomy, array $args = array(), array $names = array() ) {
			...
			} elseif ( function_exists( 'register_extended_taxonomy' ) ) {
...
if ( ! class_exists( 'Extended_CPT_Admin' ) ) {
	...
	class Extended_CPT_Admin {
		...
		public function filters() {
			...
			foreach ( $this->args['admin_filters'] as $filter_key => $filter ) {
					...
					if ( ! class_exists( $class ) ) {
						trigger_error(
							esc_html(
								sprintf(
									/* translators: %s: string */
									__( 'The "%s" class is required in order to display taxonomy filters', 'cardealer-helper' ),
									$class
								)
							),
							E_USER_WARNING
				...
				<label><?php printf( '%s:', esc_html( $filter['title'] ) ); ?>&nbsp;<input type="text" name="<?php echo esc_attr( $filter_key ); ?>" id="filter_<?php echo esc_attr( $filter_key ); ?>" value="<?php echo esc_attr( $value ); ?>" /></label>
					...
					if ( 1 === (int) count( $filter['meta_exists'] ) ) {
		...
		public function cols( array $cols ) {
			...
			foreach ( array_filter( $this->args['admin_cols'] ) as $id => $col ) {
				if ( is_string( $col ) && isset( $cols[ $col ] ) ) {
					...
					$new_cols[ $k ] = esc_html__( 'Author', 'cardealer-helper' );
				} elseif ( is_array( $col ) ) {
		...
		public function col_post_meta( $meta_key, array $args ) {
			...
			if ( isset( $args['date_format'] ) ) {
				...
				foreach ( $vals as $val ) {
					...
					if ( is_numeric( $val ) ) {
						$echo[] = gmdate( $args['date_format'], $val );
					} elseif ( ! empty( $val ) ) {
		...
		public function col_connection( $connection, array $args ) {
			...
			if ( ! $this->p2p_connection_exists( $connection ) ) {
				echo esc_html(
					sprintf(
						/* translators: %s: string */
						__( 'Invalid connection type: %s', 'cardealer-helper' ),
						$connection
					)
				);
				return;
			...
			foreach ( $wp_query->post->$field as $post ) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride
		...
		protected function get_item_title( array $item ) {
			...
			} elseif ( isset( $item['connection'] ) && isset( $item['value'] ) ) {
