...
if ( ! function_exists( 'cdhl_shedule_test_drive_action' ) ) {
	...
	function cdhl_shedule_test_drive_action() {
		...
		if ( isset( $_POST['action'] ) && 'shedule_test_drive_action' === sanitize_text_field( wp_unslash( $_POST['action'] ) ) ) {
			...
			if ( ! isset( $_POST['shedule_test_drive_nonce'] ) || ! wp_verify_nonce( sanitize_key( $_POST['shedule_test_drive_nonce'] ), 'shedule_test_drive_nonce' ) ) {
				...
				if ( $post_id ) {
					...
					$test_drive        = ( 'yes' === $test_drive ) ? esc_html__( 'Yes', 'cardealer-helper' ) : esc_html__( 'No', 'cardealer-helper' );
					$preferred_contact = ( 'email' === $preferred_contact ) ? esc_html__( 'Email', 'cardealer-helper' ) : esc_html__( 'Phone', 'cardealer-helper' );
					$fields            = array(
