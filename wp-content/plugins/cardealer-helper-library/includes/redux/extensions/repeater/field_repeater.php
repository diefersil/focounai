	...
	class ReduxFramework_pgs_repeater {
		...
		public function __construct( $field, $value, $parent ) {
			...
			if ( empty( $this->extension_dir ) ) {
				...
				// $this->extension_url = trailingslashit( get_template_directory_uri() ) . 'includes/redux/extensions/repeater/';
			}
		}
		...
		public function render() {
				...
				foreach ( $repeaters as $repeater ) {
					...
					if ( isset( $this->field['bind_title'] ) ) {
						...
						foreach ( $this->field['fields'] as $field ) {
							if ( (string) $field['id'] === (string) $this->field['bind_title'] ) {
					...
					foreach ( $this->field['fields'] as $field ) {
						if ( isset( $this->field['bind_title'] ) && (string) $field['id'] === (string) $this->field['bind_title'] ) {
					...
					if ( ! isset( $this->field['static'] ) && empty( $this->field['static'] ) ) {
						echo '<a href="javascript:void(0);" class="button deletion redux-pgs_repeaters-remove">' . esc_html__( 'Delete', 'cardealer-helper' ) . ' ' . $this->field['item_name'] . '</a>';
					}
					echo '</div>';
					echo '</fieldset>';
					echo '</table>';
			...
			if ( 0 === (int) $x || ( isset( $this->field['static'] ) && ( $x - 1 ) < $this->field['static'] ) ) {
			...
			if ( ! isset( $this->field['static'] ) && empty( $this->field['static'] ) ) {
				...
				echo '<a href="javascript:void(0);" class="button redux-pgs_repeaters-add button-primary' . $disabled . '" rel-id="' . $this->field['id'] . '-ul" rel-name="' . $this->parent->args['opt_name'] . $this->repeater_values . '[title][]">' . esc_html__( 'Add', 'cardealer-helper' ) . ' ' . $this->field['item_name'] . '</a><br/>';
			}
		...
		public function enqueue() {
			...
			$vars = array( 'option_name' => THEME_OPTIONS );
			wp_enqueue_script(
		...
		function localize( $field, $value = '' ) {
			...
			if ( isset( $field['fields'] ) && ! empty( $field['fields'] ) ) {
				...
				foreach ( $field['fields'] as $f ) {
					if ( isset( $this->field['bind_title'] ) && (string) $f['id'] === (string) $this->field['bind_title'] ) {
				...
				$var = array(
					'html'     => $var . '<a href="javascript:void(0);" class="button deletion redux-pgs_repeaters-remove">Delete </a>',
					'count'    => count( $value ),
					'sortable' => true,
					'limit'    => '',
